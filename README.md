...
| **网址导航** | **配置文件驱动**的二级工具（数据存于 `js/data/site-nav-data.js`，随项目代码提交 git；不联网、不依赖任何接口、不使用 localStorage、网页端只读）：以卡片形式展示收藏的网址（图标 / 名称 / 分类标签 / 介绍 / 链接），支持按分类筛选（全部 + 各分类）与关键词搜索；点击卡片在新标签页打开对应网站。图标留空时自动用 Google Favicon 服务按域名获取，离线或失败时回退为名称首字母色块。**增删改网站 / 分类请直接编辑配置文件** |
...
└── js/
    ...
    └── tools/                   # 各工具模块，一个文件一个工具
        ...
        ├── site-nav-tool.js     # 网址导航 · 读取配置并展示（只读，无 localStorage）
        └── data/
            └── site-nav-data.js # 网址导航 · 配置文件（分类 + 网址，手动维护、提交 git）
        # 图片工具
