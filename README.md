...
共 **7 个分类、31 个工具**，顶部导航切换分类、二级导航切换工具，部分工具内含页内三级子菜单，全程无刷新。
...
打开工具箱默认进入**首页**：以「网址导航」为核心重新设计的落地页。顶部 Hero 区带按时段问候（早上好 / 下午好 …）、醒目的搜索框与统计信息；下方按分类分节展示常用网站卡片（图标 / 名称 / 介绍），点击任意卡片在新标签页打开对应网站。顶部分类快捷标签可一键平滑滚动到对应分节；搜索框实时过滤（名称 / 介绍 / 网址）。

数据来自配置文件 `js/data/site-nav-data.js`（`window.DAIBAO_SITE_NAV`），**随项目代码提交 git、跨设备同步**；不联网、不依赖任何接口、网页端只读。**新增 / 删除 / 调整网站与分类请直接编辑该配置文件**（图标可放 `assets/icons/`，路径填相对 `index.html` 的根路径如 `assets/icons/xxx.png`；留空则自动按域名取 favicon，失败回退首字母色块）。
...
| 工具 | 说明 |
|------|------|
| **财务工具** | 由原「财务工具」一级分类整体并入办公工具，**固定排在首位**；页内三级子菜单切换增值税计算器 / 个税计算器 / 单位换算 |
...
| **米游社原神COS** | 通过 [OIAPI](https://www.oiapi.net/doc/id/148.html) 的 MihoyoCos 接口按页浏览米游社社区的原神 COS 作品（作者头像 / 昵称 / 相对时间 / 标题 / 正文 / 多图），点击图片灯箱预览。**默认不加载内容，需点击「查询作品」**才行；列表**上方与下方各有一组联动分页控件**（上一页 / 页码输入跳转 / 下一页 / 本页条数），可选每页 10 / 20 / 30 条。接口的 `page` + `limit` 仅分页、**返回体没有总数与末页字段**（深页返回 `code=1` + 空数组），故「末页」按「本页条数 < 每页条数」判定，跳到空页会提示而非报错 |
...
└── js/
    ...
    └── tools/                   # 各工具模块，一个文件一个工具
        ...
        ├── ip-location.js      # IP定位
        # 办公工具
        ├── finance-tool.js      # 财务工具外壳（页内竖直子菜单）
        ├── vat-calculator.js    # 财务工具 · 增值税计算器
        ├── tax-calculator.js    # 财务工具 · 个税计算器
        ├── unit-converter.js    # 财务工具 · 单位换算
        ├── word-count.js
        ├── case-convert.js
        ├── text-process.js
        ...
        ├── home-page.js         # 首页 · 网址导航落地页（读取配置、Hero + 分节卡片）
        └── data/
            └── site-nav-data.js # 首页 · 网址导航配置文件（分类 + 网址，手动维护、提交 git）
        # 图片工具
