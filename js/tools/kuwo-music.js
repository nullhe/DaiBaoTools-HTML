/* 酷我音乐：在线听歌 + 歌词滚动同步
 * 音频用酷我直链（升级 https 规避混合内容，<audio> 播放无需 CORS）；
 * 酷我 API 不返回歌词，统一兜底到 OIAPI(QQ) 同曲歌词做滚动同步；
 * 封面 CDN 已开放 CORS，可直接 blob 下载。 */
window.DaibaoTools = window.DaibaoTools || {};

window.DaibaoTools.createKuwoMusic = function (container) {
  var API = 'https://www.oiapi.net/api/Kuwo';
  ...
  var state = {
    ...
    detail: null, // 取词详情对象（url / picture / time / types ...）
    n: 0, // 取词序号，从 1 开始
  ...
  container.innerHTML = `
    <div class="tool-card">
      <div class="tool-section">
        ...
        <p class="tool-quiet">数据来源 OIAPI「酷我音乐」接口。音频来自酷我；酷我未提供歌词，已自动使用 QQ 同曲歌词做滚动同步。仅供个人学习使用。</p>
      </div>

      <div class="qm-search">
        <input type="text" class="tool-input" id="kwKeyword" placeholder="输入歌名或歌手，例如：周杰伦 / 晴天" />
        <button class="tool-btn primary" id="kwSearchBtn">搜索</button>
      </div>

      <div id="kwStatus" class="qm-status" hidden></div>
      <div id="kwResults" class="qm-results" hidden></div>

      <div id="kwDetail" class="mm-detail" hidden>
        <div class="qm-song-head">
          <div class="qm-cover" id="kwCover"></div>
          <div class="qm-song-info">
            <div class="qm-song-name" id="kwName"></div>
            <div class="qm-song-sub" id="kwSub"></div>
            <span class="mm-vip" id="kwVip" hidden>VIP</span>
          </div>
        </div>

        <audio id="kwAudio" class="mm-audio" controls preload="metadata"></audio>

        <div class="mm-quality" id="kwQuality"></div>

        <div class="qm-toolbar">
          <label class="qm-switch" title="控制歌词行前是否显示播放时间">
            <input type="checkbox" id="kwShowTime" checked />
          ...
          <div class="qm-toolbar-right">
            <button class="tool-btn" id="kwCoverBtn">封面下载</button>
            <button class="tool-btn" id="kwCopyBtn">复制歌词</button>
            <button class="tool-btn primary" id="kwDownloadBtn">下载 TXT</button>
          </div>
        </div>

        <div id="kwLyric" class="qm-lyric"></div>
        <div class="mm-src" id="kwSrc" hidden></div>
      </div>
    </div>`;

  var keywordEl = container.querySelector('#kwKeyword');
  var searchBtn = container.querySelector('#kwSearchBtn');
  var statusEl = container.querySelector('#kwStatus');
  var resultsEl = container.querySelector('#kwResults');
  var detailEl = container.querySelector('#kwDetail');
  var coverEl = container.querySelector('#kwCover');
  var nameEl = container.querySelector('#kwName');
  var subEl = container.querySelector('#kwSub');
  var vipEl = container.querySelector('#kwVip');
  var audioEl = container.querySelector('#kwAudio');
  var qualityEl = container.querySelector('#kwQuality');
  var showTimeEl = container.querySelector('#kwShowTime');
  var copyBtn = container.querySelector('#kwCopyBtn');
  var downloadBtn = container.querySelector('#kwDownloadBtn');
  var coverBtn = container.querySelector('#kwCoverBtn');
  var lyricEl = container.querySelector('#kwLyric');
  var srcEl = container.querySelector('#kwSrc');
  ...
  // 酷我 time 是秒数（如 "269"），转 mm:ss 方便展示
  ...
  function applyLrc(text, source) {
    ...
    if (state.lyricSource) {
      srcEl.hidden = false;
      srcEl.textContent =
        '歌词来源：酷我未提供歌词，已自动使用 QQ 同曲歌词（音频仍为酷我）';
    }
  ...
  function finishNoLyric() {
    ...
    state.raw = '';
    renderLyric();
    setStatus('该歌曲暂无歌词内容（酷我未提供，QQ 也未找到同曲）', 'warn');
  }
  ...
  function renderResults() {
    ...
    state.results.forEach(function (song, i) {
      ...
      time.className = 'qm-result-time';
      time.textContent = fmtDuration(song.time);
  ...
  function renderHead(d) {
    ...
    var sub = [singer, fmtDuration(d && d.time)].filter(Boolean).join(' · ');
  ...
  function renderQuality(d) {
    ...
    types.forEach(function (t, idx) {
      ...
      b.type = 'button';
      b.className = 'mm-qbtn' + (idx + 1 === state.br ? ' active' : '');
      b.textContent = t.level + ' ' + t.bitrate + (t.format ? ' ' + t.format : '');
  ...
  function renderLyric() {
    ...
    if (!state.lines.length) {
      lyricEl.innerHTML =
        '<div class="tool-empty"><div class="tool-empty-icon">🎼</div><h3>该歌曲暂无歌词</h3><p>酷我未提供歌词，且未找到 QQ 同曲歌词。</p></div>';
      return;
    }
  ...
  function applyAudio(d) {
    ...
    // 酷我音频是 http 直链，升级为 https 规避混合内容（实测 https 同样可用）
  ...
  function setBr(br) {
    if (br === state.br) return;
    state.br = br;
    renderQuality(state.detail);
    setStatus('正在切换音质…', 'info');
    requestDetail(state.keyword, state.n, br)
      .then(function (d) {
        state.detail = d;
        applyAudio(d);
        setStatus('音质已切换', 'ok');
  ...
  function pickSong(i) {
    ...
    requestDetail(state.keyword, state.n, '')
      .then(function (d) {
        state.detail = d;
        // 默认高亮当前详情音质在 types 中的索引
        var idx = -1;
        for (var k = 0; k < (d.types || []).length; k++) {
          if (d.types[k].level === d.level) {
            idx = k;
            break;
          }
        }
        state.br = idx >= 0 ? idx + 1 : (d.types && d.types.length ? 1 : 0);
        renderHead(d);
        applyAudio(d);
        renderQuality(d);
  ...
  function buildText() {
    ...
    parts.push('歌词来源：QQ 同曲兜底（酷我未提供）');
    parts.push('音频来源：酷我音乐 (OIAPI)');
    parts.push('');
    parts.push('--------------------');
    parts.push('');
    if (!state.parsed) {
  ...
  // 酷我封面 CDN 已开放 CORS，可直接 blob 下载；失败则新标签打开兜底
